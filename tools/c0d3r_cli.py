...
def _is_self_query(prompt: str) -> bool:
    lower = (prompt or "").lower()
    if "c0d3r" in lower or "coder" in lower:
        return True
    cues = (
        "this cli",
        "this tool",
...
def _is_self_upgrade_request(prompt: str) -> bool:
    lower = (prompt or "").lower()
    if "c0d3r" in lower or "coder" in lower or "this cli" in lower or "this tool" in lower:
        if any(term in lower for term in ("upgrade", "update", "improve", "refactor", "fix", "patch", "self")):
            return True
    return False


def _self_context_block(workdir: Path, *, session_id: str | None = None) -> str:
    try:
        cli_path = Path(__file__).resolve()
    except Exception:
        cli_path = Path("tools/c0d3r_cli.py")
    entrypoints = [
        PROJECT_ROOT / "bin" / "c0d3r",
        PROJECT_ROOT / "bin" / "c0d3r.cmd",
    ]
    try:
        system_map = _runtime_path("system_map.json")
        if not system_map.exists() or (time.time() - system_map.stat().st_mtime) > 1800:
            _update_system_map(PROJECT_ROOT)
    except Exception:
        pass
    lines = [
        "Self reference:",
        f"- cli_script: {cli_path}",
        f"- runtime_root: {_runtime_root()}",
        f"- project_root: {PROJECT_ROOT}",
    ]
    existing = [str(p.resolve()) for p in entrypoints if p.exists()]
    if existing:
        lines.append(f"- entrypoints: {', '.join(existing)}")
    code_summary = _load_code_memory_summary(max_chars=1800)
    if code_summary:
...
def _self_upgrade_guidance(*, python_exec: str, project_root: Path) -> str:
    checks_cmd = (
        f"\"{python_exec}\" -m py_compile tools/c0d3r_cli.py tools/c0d3r_session.py "
        "tools/c0d3r_obstacle_course.py tools/c0d3r_turing_test.py services/side_loaded_memory.py "
        "services/short_term_memory.py services/long_term_memory.py services/conversation_memory.py"
    )
    help_cmd = f"\"{python_exec}\" tools/c0d3r_cli.py --help"
    return "\n".join(
        [
            "Self-upgrade protocol (scientific/engineering mode):",
...
def _self_upgrade_hypotheses_path() -> Path:
    return _runtime_path("self_upgrade_hypotheses.json")


def _collect_self_upgrade_hypotheses(session: "C0d3rSession", prompt: str, *, min_count: int = 10) -> list[dict]:
    if not session or not prompt:
        return []
    system = _control_system_prompt(
        "Return ONLY JSON with key: upgrades (list). "
        "Each item must be: {id, hypothesis, expected_impact, validation, guardrails}. "
        f"Generate at least {min_count} upgrades using rigorous scientific/engineering vernacular. "
        "guardrails must explicitly state that adaptive reasoning, feedback loops, and context injection "
        "must not be replaced by rigid one-off functions."
    )
    query = (
        "Self-upgrade ideation request:\n"
        f"{prompt}\n\n"
        "Provide upgrade hypotheses with measurable outcomes and validation steps."
    )
    upgrades: list[dict] = []
    for _ in range(2):
        try:
            raw = session.send(prompt=query, stream=False, system=system)
        except Exception:
            break
        payload = _safe_json(raw or "")
        if isinstance(payload, dict):
            upgrades = payload.get("upgrades") or []
        if isinstance(upgrades, list) and len(upgrades) >= min_count:
            break
        upgrades = []
    cleaned: list[dict] = []
    if isinstance(upgrades, list):
        for idx, item in enumerate(upgrades):
            if not isinstance(item, dict):
                continue
            cleaned.append({
                "id": item.get("id") or f"upgrade_{idx + 1}",
                "hypothesis": str(item.get("hypothesis") or "").strip(),
                "expected_impact": str(item.get("expected_impact") or "").strip(),
                "validation": str(item.get("validation") or "").strip(),
                "guardrails": str(item.get("guardrails") or "").strip(),
            })
            if len(cleaned) >= min_count:
                break
    if cleaned:
        try:
            path = _self_upgrade_hypotheses_path()
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text(json.dumps({"generated_at": time.strftime("%Y-%m-%d %H:%M:%S"), "upgrades": cleaned}, indent=2), encoding="utf-8")
        except Exception:
            pass
    return cleaned
...
def _side_loaded_index(session_id: str | None, workdir: Path) -> object:
    key = f"{session_id or 'global'}::{workdir}"
    cached = _SIDE_MEMORY_CACHE.get(key)
    if cached is not None:
        return cached
    try:
        from services.side_loaded_memory import SideLoadedMemoryIndex
    except Exception:
        return None
    index = SideLoadedMemoryIndex(
        runtime_root=_runtime_root(),
        session_id=session_id,
        workdir=workdir,
        env=_env_signature(),
    )
    _SIDE_MEMORY_CACHE[key] = index
    return index
...
def _touch_search_memory(record_id: str, *, path: str | None = None, workdir: Path | None = None, session_id: str | None = None) -> None:
    workdir = workdir or Path.cwd()
    index = _side_loaded_index(session_id, workdir)
    if index is not None:
        try:
            index.touch(record_id, source_path=path)
            return
        except Exception:
            pass
    try:
...
def _search_memory_hint(prompt: str, *, session_id: str | None = None, workdir: Path | None = None) -> str:
    prompt = (prompt or "").strip()
    if not prompt:
        return ""
    workdir = workdir or Path.cwd()
...
def _side_loaded_router(session, prompt: str) -> dict:
    ...
    system = _control_system_prompt(
        ...
        "Formulate empirical retrieval hypotheses: use search_queries to locate files/paths/scripts and "
        "env_queries to match known command fixes. "
        "Keep each query under 8 words and tied to observable artifacts."
    )
...
def _decide_recall_scope(
    ...
    system = _control_system_prompt(
        ...
        "Treat this as an evidence-selection problem: choose short if the short-term summary or recent "
        "transcript likely contains the answer; choose long only when older history is required. "
        "Provide the query string that should be searched."
    )
    prompt_block = (
...
def _collect_context_injections(
    inline_entries: list[str] | None,
    file_entries: list[str] | None,
    workdir: Path,
) -> list[str]:
    blocks: list[str] = []
    for entry in inline_entries or []:
        text = str(entry).strip()
        if text:
            blocks.append(text)
    for raw in file_entries or []:
        if not raw:
            continue
        try:
            ...
            if content:
                blocks.append(f"[{candidate.name}]\n{content}")
        except Exception:
            continue
    return blocks
...
def _framework_decision(session: C0d3rSession, prompt: str, workdir: Path) -> dict:
    system = _control_system_prompt(
        ...
        "Select a concrete framework stack using evidence-based reasoning and provide shell commands to scaffold it. "
...
def _decide_new_project(session: C0d3rSession, prompt: str, workdir: Path) -> bool:
    ...
    system = _control_system_prompt(
        ...
        "Treat this as a binary, evidence-based decision. Choose true only when the user explicitly asks "
...
def _decide_project_name(session: C0d3rSession, prompt: str, workdir: Path) -> str:
    ...
    system = _control_system_prompt(
        ...
        "Pick a short, unique, branding-style name for a new project folder.\n"
        "Use CamelCase with no spaces. Do not include file extensions. Favor memorable, low-collision names."
    )
    try:
        raw = session.send(prompt=f"CWD: {workdir}\nTask: {prompt}", stream=False, system=system)
        payload = _safe_json(raw)
...
def _decide_simple_task(session: C0d3rSession, prompt: str) -> bool:
    ...
    system = _control_system_prompt(
        ...
        "Use conservative judgment based on empirical task complexity."
    )
    try:
        raw = _call_with_timeout(
            session._safe_send,
            timeout_s=float(os.getenv("C0D3R_SIMPLE_SCREEN_TIMEOUT_S", "10") or "2"),
...
def _intent_for_step(step: int, *, step_failed: bool, last_error: str = "", output_request: bool = False) -> str:
    if output_request:
        return "Perform error analysis on prior output chunks to remove ambiguity before proposing new commands."
    if step_failed:
        return "Diagnose the failure root cause; propose corrective commands and verification checks."
    if step == 1:
        return "Formulate the initial experimental command plan and validation checks to satisfy the objective."
    return "Iterate with the next experimental commands and verification checks to complete the objective."
...
def _heartbeat_message(*, command: str, cwd: Path, elapsed_s: float, last_line: str) -> str:
    ...
    if not _HEARTBEAT_USE_MODEL or _HEARTBEAT_SESSION is None or not _HEARTBEAT_MODEL_ID:
        return fallback
    system = (
        "Return ONE short, empirically grounded line (<=120 chars) confirming the task is still running. "
        "No extra formatting."
    )
    prompt = (
        "Task heartbeat.\n"
...
def _stream_subprocess(
    *,
    ...
    timeout_s = float(timeout_s) if timeout_s is not None else float(os.getenv("C0D3R_CMD_TIMEOUT_S", "600") or "180")
...
def _run_tool_loop_v2(
    ...
    *,
    ...
    petals = PetalManager()
    petal_effects: dict[str, object] = {}
    petal_constraints: list[dict] = []
    petal_note_cursor = 0
    upgrade_hypotheses: list[dict] = []
    if self_upgrade_requested:
        upgrade_hypotheses = _collect_self_upgrade_hypotheses(session, base_request, min_count=10)
    summary_root = _find_nearest_project_root(current_cwd) or current_cwd
    initial_git_snapshot = _snapshot_git(summary_root, run_command) if run_command else ""
    executed_commands: list[dict] = []
    executed_checks: list[dict] = []
    executed_tests: list[dict] = []
    written_paths: list[str] = []
    written_seen: set[str] = set()

    def _refresh_petals(reason: str = "") -> None:
        nonlocal petal_effects, petal_constraints, petal_note_cursor
        updated = petals.refresh()
        overrides = petals.consume_overrides()
        if overrides:
            petals.update_from_directives(overrides)
            history.append(f"[petal] overrides applied ({len(overrides)})")
        if petal_note_cursor < len(user_notes):
            new_notes = user_notes[petal_note_cursor:]
            petal_note_cursor = len(user_notes)
            directives: list[dict] = []
            for note in new_notes:
                directives.extend(_extract_petal_directives(note))
            if directives:
                petals.update_from_directives(directives)
                history.append(f"[petal] note directives applied ({len(directives)})")
        petal_constraints = petals.active_constraints()
        if petal_constraints:
            plan = _petal_action_plan(session, petal_constraints, base_request)
            petal_effects = _apply_petal_plan(plan)
        else:
            petal_effects = {}
        if updated and reason:
            history.append(f"[petal] refreshed: {reason}")
    ...
    def _run_self_upgrade_checks() -> tuple[bool, str]:
        python_exec = sys.executable
        project_root = PROJECT_ROOT
        compile_targets = [
            "tools/c0d3r_cli.py",
            "tools/c0d3r_session.py",
            "tools/c0d3r_obstacle_course.py",
            "tools/c0d3r_turing_test.py",
            "services/side_loaded_memory.py",
            "services/short_term_memory.py",
            "services/long_term_memory.py",
            "services/conversation_memory.py",
        ]
        compile_cmd = f"\"{python_exec}\" -m py_compile " + " ".join(compile_targets)
        checks = [
            ("py_compile", compile_cmd),
            ("cli_help", f"\"{python_exec}\" tools/c0d3r_cli.py --help"),
        ]
        summary: list[str] = []
        ok = True
        for idx, (label, cmd) in enumerate(checks, start=1):
            _ui_write(f"\n[self-check {idx}/{len(checks)}] {label}\n")
            _ui_write(f"cwd: {project_root}\n")
            _ui_write(f"command: {cmd}\n")
            res = _stream_subprocess(
                command=cmd,
                cwd=project_root,
                shell="auto",
                timeout_s=float(os.getenv("C0D3R_SELF_CHECK_TIMEOUT_S", "180") or "180"),
                log_prefix=f"self_upgrade_{idx}",
                purpose=f"self-upgrade {label}",
                raw_command=cmd,
            )
            ...
            _record_execution("check", f"self-check {label}", cmd, project_root, exit_code, timed_out, duration_s)
            success = bool(exit_code == 0 and not timed_out)
            summary.append(f"{label}={'ok' if success else 'fail'}")
            important = _summarize_lines(tail or head, max_lines=8)
            if important:
                _ui_write("Output (most relevant):\n" + important + "\n")
            history.append(
                "\n".join(
                    [
                        f"[self_upgrade_check] {label} exit={exit_code} timeout={timed_out}",
                        f"command: {cmd}",
                        f"cwd: {project_root}",
                        f"head:\n{head}",
                        f"tail:\n{tail}",
    ...
    def _parse_git_status(snapshot: str) -> list[str]:
        files: list[str] = []
        for line in (snapshot or "").splitlines():
            raw = line.rstrip()
            if not raw:
                continue
            match = re.match(r"^[MARCDU?]{1,2}\s+(.*)$", raw)
            path = match.group(1).strip() if match else raw.strip()
            if "->" in path:
    ...
    def _format_path(path: Path) -> str:
        try:
            rel = path.resolve().relative_to(summary_root.resolve())
            return str(rel)
        except Exception:
            return str(path)

    def _record_execution(kind: str, purpose: str, cmd: str, cwd: Path, exit_code: int, timed_out: bool, duration_s: float) -> None:
        entry = {
            "kind": kind,
            "purpose": purpose,
            "command": cmd,
        ...
        if kind == "command":
            ...
            executed_checks.append(entry)
        if _is_test_command(cmd, purpose):
            executed_tests.append(entry)
        if _is_write_command(cmd):
            inferred = _infer_written_path(cmd, cwd)
            candidates: list[Path] = []
            if inferred:
                candidates.append(inferred)
            else:
                for target in _command_targets(cmd):
                    try:
            ...
            for candidate in candidates:
                try:
                    ...
                    key = str(candidate)
                if key in written_seen:
                    continue
                written_seen.add(key)
                written_paths.append(_format_path(candidate))

    def _is_test_command(cmd: str, purpose: str = "") -> bool:
        lower = (cmd or "").lower()
        if "test" in (purpose or "").lower():
            return True
        patterns = (
            r"\bpytest\b",
            r"\bunittest\b",
    ...
    def _build_execution_summary(final_text: str) -> str:
        lines: list[str] = []
        lines.append("Execution summary (actual work performed):")
        lines.append(f"- project_root: {summary_root}")
        if executed_commands:
            lines.append(f"- commands_executed: {len(executed_commands)}")
            for item in executed_commands:
                status = "ok" if item["success"] else "fail"
                lines.append(f"  - {item['purpose']} [{status}] {item['command']} (cwd={item['cwd']})")
        else:
            lines.append("- commands_executed: none")
        if executed_checks:
            lines.append(f"- checks_executed: {len(executed_checks)}")
            for item in executed_checks:
                status = "ok" if item["success"] else "fail"
                lines.append(f"  - {item['purpose']} [{status}] {item['command']} (cwd={item['cwd']})")
        else:
            lines.append("- checks_executed: none")
        if executed_tests:
            unique_tests = []
            seen_tests = set()
            for item in executed_tests:
                cmd = item["command"]
                if cmd in seen_tests:
                    continue
                seen_tests.add(cmd)
                unique_tests.append(item)
            lines.append(f"- tests_executed: {len(unique_tests)}")
            for item in unique_tests:
                status = "ok" if item["success"] else "fail"
                lines.append(f"  - {item['command']} [{status}]")
        else:
            lines.append("- tests_executed: none")
        delta_files: list[str] = []
        if initial_git_snapshot and run_command:
            final_git_snapshot = _snapshot_git(summary_root, run_command)
            initial_files = set(_parse_git_status(initial_git_snapshot))
            final_files = set(_parse_git_status(final_git_snapshot))
            delta_files = sorted([f for f in final_files if f not in initial_files])
        combined_files = []
        combined_seen = set()
        for item in written_paths + delta_files:
            if item in combined_seen:
                continue
            combined_seen.add(item)
            combined_files.append(item)
        if combined_files:
            lines.append(f"- files_written_or_modified: {len(combined_files)}")
            for path in combined_files:
                lines.append(f"  - {path}")
        else:
            lines.append("- files_written_or_modified: none detected")
        if upgrade_hypotheses:
            lines.append(f"- self_upgrade_hypotheses: {len(upgrade_hypotheses)}")
            for item in upgrade_hypotheses:
                hypothesis = item.get("hypothesis") or ""
                validation = item.get("validation") or ""
                guardrails = item.get("guardrails") or ""
                snippet = hypothesis or item.get("id") or "upgrade"
                tail = f" | validation: {validation}" if validation else ""
                if guardrails:
                    tail = (tail + f" | guardrails: {guardrails}") if tail else f"guardrails: {guardrails}"
                lines.append(f"  - {snippet}{tail}")
        summary = "\n".join(lines).strip()
        if not summary:
            return final_text
        if final_text:
    ...
    for step in range(1, max_steps + 1):
        ...
        _refresh_petals("step start")

        if petal_effects.get("pause_for_input") and allow_interrupt:
            try:
                note = _prompt_local_input("[petal] paused for input (enter override or blank to continue): ")
            except Exception:
                note = ""
            if note:
        ...
        search_hint = _search_memory_hint(base_request, session_id=session_id, workdir=current_cwd)
        if side_env_queries:
        ...
        if side_search_queries:
            for q in side_search_queries:
                hint = _search_memory_hint(str(q), session_id=session_id, workdir=current_cwd)
        ...
        self_block = ""
        if _is_self_query(base_request) and "Self reference:" not in context_block:
            self_context = _self_context_block(current_cwd, session_id=session_id)
            if self_context:
                self_block = "Self context:\n" + self_context
        self_upgrade_block = ""
        if self_upgrade_requested:
            self_upgrade_block = _self_upgrade_guidance(
                python_exec=sys.executable,
                project_root=PROJECT_ROOT,
            )
        upgrades_block = ""
        if upgrade_hypotheses:
            try:
                upgrades_block = "Self-upgrade hypotheses:\n" + json.dumps(upgrade_hypotheses, indent=2)[:4000]
            except Exception:
                upgrades_block = "Self-upgrade hypotheses: (unavailable)"

        petal_block = ""
        petal_instructions: list[str] = []
        if petal_constraints:
            try:
                petal_block = "Petal constraints:\n" + json.dumps(petal_constraints, indent=2)
            except Exception:
                petal_block = "Petal constraints: (unavailable)"
        if petal_effects:
            petal_instructions.append(f"Petal effects: {json.dumps(petal_effects)}")
            if petal_effects.get("force_tests"):
                petal_instructions.append("Include at least one test/check command in checks[].")
            if petal_effects.get("force_file_edits"):
                petal_instructions.append("Prefer file-editing commands that produce concrete artifacts.")
            if petal_effects.get("pause_for_input"):
                petal_instructions.append("Pause for user confirmation before finalizing.")
            if petal_effects.get("force_tool_loop"):
                petal_instructions.append("Stay in tool-loop mode until work is complete.")
        if petal_instructions:
            petal_block = (petal_block + "\n\nPetal instructions:\n" + "\n".join(f"- {item}" for item in petal_instructions)).strip()
        ...
        tool_prompt = (
            ...
            + ("\n\n" + self_block if self_block else "")
            + ("\n\n" + self_upgrade_block if self_upgrade_block else "")
            + ("\n\n" + upgrades_block if upgrades_block else "")
            + ("\n\n" + petal_block if petal_block else "")
            + "\n\nObjective:\n"
            + objective
            + "\n\nIntent:\n"
            + intent
        ...
        if petal_effects.get("model_override"):
            selected_model = str(petal_effects.get("model_override") or selected_model)
        if selected_model:
        ...
        if final and not commands and not checks and not needs_terminal:
            return _build_execution_summary(final)
        ...
        for i, cmd in enumerate(commands[:max_commands_per_step], start=1):
            ...
            if timeout_s is None and petal_effects.get("timeout_override") is not None:
                timeout_s = petal_effects.get("timeout_override")
            try:
            ...
            if memory_hit:
                ...
                _touch_search_memory(
                    memory_hit.get("id") or "",
                    path=memory_hit.get("source_path"),
                    workdir=current_cwd,
                    session_id=session_id,
                )
                executed = False
            ...
            success = bool(exit_code == 0 and not timed_out)
            if executed:
                _record_execution("command", purpose, raw_command, cwd, exit_code, timed_out, duration_s)
        ...
        if not step_failed and not user_interrupted and checks:
            ...
            for j, chk in enumerate(checks[:max_commands_per_step], start=1):
                ...
                success = bool(exit_code == 0 and not timed_out)
                if executed:
                    _record_execution("check", purpose, raw_command, cwd, exit_code, timed_out, duration_s)
        ...
        if final and not step_failed and self_upgrade_requested and not self_upgrade_verified:
            ...
            ok, summary = _run_self_upgrade_checks()
            history.append(f"[self-upgrade] {summary}")
            if ok:
                self_upgrade_verified = True
            else:
                step_failed = True
                last_error = "self-upgrade checks failed"
                history.append("[next] self-upgrade checks failed; fix and re-run.")

        if petal_constraints and not user_interrupted:
            outcome_score = 1.0 if not step_failed else 0.0
            petals.record_outcome(not step_failed, outcome_score, note=last_error)
    ...
    return _build_execution_summary(
        "I couldn't complete the task within the execution budget.\nLast signals:\n" + tail[-2500:]
    )
...
def _math_grounding_block(session: C0d3rSession, prompt: str, workdir: Path) -> str:
    ...
    system = _control_system_prompt(
        ...
        "identify missing constants/relations. Provide units for equations and "
        "define symbols explicitly so they are measurable."
    )
    ...
    if research_notes:
        ...
        system2 = _control_system_prompt(
...
def _force_file_ops(session: C0d3rSession, prompt: str, workdir: Path, *, base_root: Path | None = None) -> list[Path]:
    ...
    system = _control_system_prompt(
        ...
        "At least 2 file_ops. Use deterministic file operations only where the task is deterministic."
    )
    if _requires_new_projects_dir(prompt):
        system += (
...
def _force_commands(session: C0d3rSession, prompt: str) -> list[str]:
    system = _control_system_prompt(
        ...
        "Provide 1-5 concrete shell commands that execute the task with measurable outcomes. "
...
def _force_local_extract_commands(session: C0d3rSession, prompt: str, target: str) -> list[str]:
    system = _control_system_prompt(
        ...
        "Use platform-appropriate commands (PowerShell on Windows) and verify extraction. "
...
def _critical_thinking_checkpoint(session: C0d3rSession, prompt: str, history: List[str]) -> dict:
    system = _control_system_prompt(
        ...
        "and decide if code edits/tests are required now."
    )
    try:
        context = "\n".join(history[-6:])
        raw = session.send(
...
class PetalManager:
    ...
    def __init__(self) -> None:
        ...
        self.override_path = _runtime_path("petal_override.json")
        self.state = {
            "constraints": [],
            "active": {},
            "cooldowns": {},
    ...
    def update_from_directives(self, directives: list[dict]) -> None:
        if not directives:
            return
        existing = {str(item.get("name")): item for item in (self.state.get("constraints") or []) if isinstance(item, dict)}
        for directive in directives:
            if not isinstance(directive, dict):
                continue
            name = str(directive.get("name") or "").strip() or f"petal_{len(existing) + 1}"
            merged = dict(existing.get(name) or {})
            merged.update(directive)
            merged["name"] = name
            existing[name] = merged
        self.state["constraints"] = list(existing.values())
        self.save()
    ...
    def consume_overrides(self) -> list[dict]:
    ...
    def active_constraints(self) -> list[dict]:
        active: list[dict] = []
        now = time.time()
        for constraint in self.state.get("constraints", []) or []:
            if not isinstance(constraint, dict):
                continue
            name = str(constraint.get("name") or "")
            if name and not self.active(name):
                continue
            ttl_raw = constraint.get("ttl")
            if ttl_raw and "ttl_steps" not in constraint and "ttl_seconds" not in constraint:
                ttl_text = str(ttl_raw).strip().lower()
                if ttl_text.startswith("steps:"):
                    try:
                        constraint["ttl_steps"] = int(ttl_text.split(":", 1)[1])
                    except Exception:
                        pass
                elif ttl_text.endswith("s") and ttl_text[:-1].isdigit():
                    constraint["ttl_seconds"] = float(ttl_text[:-1])
            ttl_steps = constraint.get("ttl_steps")
            if ttl_steps is not None:
                try:
                    remaining = int(constraint.get("remaining_steps", ttl_steps))
                except Exception:
                    remaining = None
                if remaining is not None and remaining <= 0:
                    self.disable(name, "ttl expired")
                    continue
                constraint["remaining_steps"] = remaining
            ttl_seconds = constraint.get("ttl_seconds")
            if ttl_seconds is not None:
                if "started_at" not in constraint:
                    constraint["started_at"] = now
                started = float(constraint.get("started_at") or constraint.get("ts") or now)
                if now - started > float(ttl_seconds):
                    self.disable(name, "ttl expired")
                    continue
            active.append(constraint)
        return active

    def record_outcome(self, success: bool, score: float, note: str = "") -> None:
        metrics = self.state.setdefault("metrics", {})
        for constraint in self.active_constraints():
            name = str(constraint.get("name") or "")
            if not name:
                continue
            entry = metrics.get(name) or {"attempts": 0, "successes": 0, "score": 0.0}
            entry["attempts"] = int(entry.get("attempts") or 0) + 1
            entry["successes"] = int(entry.get("successes") or 0) + (1 if success else 0)
            prior_score = float(entry.get("score") or 0.0)
            entry["score"] = prior_score * 0.7 + float(score) * 0.3
            entry["last_note"] = note
            entry["last_updated"] = time.time()
            metrics[name] = entry
            try:
                remaining = int(constraint.get("remaining_steps")) if constraint.get("remaining_steps") is not None else None
            except Exception:
                remaining = None
            if remaining is not None:
                constraint["remaining_steps"] = max(0, remaining - 1)
            attempts = entry["attempts"]
            success_rate = entry["successes"] / max(1, attempts)
            if attempts >= 3 and success_rate < 0.4:
                self.disable(name, f"auto-disabled: success_rate={success_rate:.2f}")
...
def _extract_petal_directives(prompt: str) -> list[dict]:
    ...
    for line in prompt.splitlines():
        ...
        if not line_strip:
            continue
        if line_strip.lower().startswith(("petal:", "workflow:", "override:")):
            ...
            if raw:
                ...
                if isinstance(payload, dict):
                    ...
                    directives.extend([item for item in payload if isinstance(item, dict)])
                else:
                    directives.append(
                        {
                            "name": f"user_override_{abs(hash(raw)) % 10000}",
                            "type": "user_override",
                            "payload": raw,
                            "ttl": "session",
                            "priority": 6,
                            "rationale": "user override directive",
                        }
                    )
    ...
    if any(phrase in lower for phrase in ("pause", "stop and ask", "wait for me", "check with me")):
        directives.append({
            ...
            "ttl": "steps:1",
            "priority": 7,
            "rationale": "user requested pause for input",
        })
    if any(phrase in lower for phrase in ("prioritize tests", "run tests first", "tests before")):
        directives.append({
            ...
            "ttl": "session",
            "priority": 5,
            "rationale": "user requested tests to be prioritized",
        })
    if any(phrase in lower for phrase in ("focus on edits", "edit files", "update files")):
        directives.append({
            "name": "prioritize_file_edits",
            "type": "prioritize_file_edits",
            "payload": prompt.strip(),
            "ttl": "session",
            "priority": 4,
            "rationale": "user requested file edits focus",
        })
    return directives
...
def _build_tech_matrix(session: C0d3rSession, prompt: str) -> dict:
    ...
    system = _control_system_prompt(
        ...
        "Use precise engineering terminology and measurable boundaries."
    )
...
def _compile_tech_outline(session: C0d3rSession, matrix: dict) -> dict:
    system = _control_system_prompt(
        ...
        "Use the matrix to propose a concrete software outline with explicit verification steps."
    )
...
def _petal_action_plan(session: C0d3rSession, constraints: list[dict], prompt: str) -> dict:
    ...
    system = _control_system_prompt(
        ...
        "Do not invent capabilities not listed; justify choices via measurable outcomes."
    )
    try:
        raw = session.send(
...
def _run_repl(
    ...
    *,
    ...
    session_id = getattr(session, "session_id", "") if session else ""
    runtime_root = _runtime_root()
    memory = ShortTermMemory(runtime_root=runtime_root, session_id=session_id)
    memory_long = LongTermMemory(runtime_root=runtime_root)
    summary_bundle = _load_summary_bundle(session_id=session_id)
    ...
    while True:
        ...
        self_context = ""
        if _is_self_query(user_prompt_inlined):
            self_context = _self_context_block(workdir, session_id=session_id)
            if self_context:
                system_context_block = f"{self_context}\n{system_context_block}" if system_context_block else self_context
                context = f"{self_context}\n{context}" if context else self_context
        ...
        system = (
            "Role: You are a senior systems engineer operating with scientific rigor. "
            "Use empirical evidence, measurable checks, and iterative feedback loops. "
            "Prefer context injection over rigid heuristics except for deterministic operations. "
        ...
        intent = (
            "Respond to the user request using empirical reasoning. If terminal actions are required, "
            "set needs_terminal=true and provide commands/checks with verification. "
...
def _refresh_pricing_cache(session: C0d3rSession, header: "HeaderRenderer", model_id: str) -> None:
    ...
    system = _control_system_prompt(
        ...
        "Use the latest month published from the research evidence. If unknown, return null values."
    )
    try:
...
def _decide_mode(session: C0d3rSession, prompt: str, *, default_scientific: bool, default_tool_loop: bool) -> str:
    ...
    if _is_conversation_prompt(prompt):
        return "direct"
    router_system = _control_system_prompt(
        ...
        "Choose direct for pure discussion. Use empirical criteria and avoid guessing."
    )
    router_prompt = f"User request: {prompt}"
    try:
        response = session.send(router_prompt, stream=False, system=router_system)
        payload = _safe_json(response)
...
def _plan_execution(session: C0d3rSession, prompt: str) -> dict:
    ...
    system = _control_system_prompt(
        ...
        "Choose the minimal set of steps needed to complete the task using evidence-based criteria. "
...
def _screen_tools(session: C0d3rSession, prompt: str, *, timeout_s: float | None = None) -> dict:
    ...
    system = _control_system_prompt(
        ...
        "Enable research only if necessary for accuracy; justify with measurable needs."
    )
    try:
...
def _rigorous_auto_enabled() -> bool:
    return os.getenv("C0D3R_RIGOROUS_AUTO", "0").strip().lower() in {"1", "true", "yes", "on"}
...
def _resolve_unbounded_request(session: C0d3rSession, prompt: str) -> dict | None:
    ...
    try:
        ...
        if prior_matrix:
            ...
            reuse_system = _control_system_prompt(
...
def _is_write_command(cmd: str) -> bool:
    ...
    if any(token in lower for token in write_tokens):
        return True
    if ">" in lower and "2>" not in lower:
        return True
    return False
    ...
    try:
        ...
        if ">" in lower and "2>" not in lower:
            ...
            if len(parts) >= 2:
                ...
                if target:
                    return (workdir / target).resolve() if not Path(target).is_absolute() else Path(target).resolve()
        if " -path " in lower:
...
def _command_timeout_s(cmd: str) -> int:
    try:
        base = int(os.getenv("C0D3R_CMD_TIMEOUT_S", "600") or "180")
    except Exception:
        base = 180
    lower = cmd.lower()
