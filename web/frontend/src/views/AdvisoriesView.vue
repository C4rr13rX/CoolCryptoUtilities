<template>
  <div class="advisories-view">
    <header class="view-header">
      <h1>Advisories</h1>
      <p>Actionable recommendations generated by the pipeline.</p>
    </header>
    <section class="panel">
      <header>
        <h2>Open Items</h2>
        <span class="caption">{{ openAdvisories.length }} active</span>
      </header>
      <ul class="advisory-list">
        <li v-for="advisory in openAdvisories" :key="advisory.id || advisory.ts">
          <div class="pill" :class="advisory.severity">{{ advisory.severity }}</div>
          <div class="content">
        ...
        <li v-if="openAdvisories.length === 0" class="empty">All advisories resolved.</li>
      </ul>
    </section>
    ...
    <section class="panel">
      ...
      <ul class="advisory-list history">
        ...
        <li v-if="resolvedAdvisories.length === 0" class="empty">No historical advisories logged yet.</li>
      </ul>
    </section>
  </div>
</template>
