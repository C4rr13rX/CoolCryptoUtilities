...
class MarketDataStream:
    ...
    # Throttle DNS outage logs per symbol to avoid duplicate warnings when multiple streams are active.
    _dns_log_by_symbol: Dict[str, float] = {}
    ...
    def __init__(
        ...
        *,
        ...
        self._offline_sample_interval = max(
            self.rest_poll_interval,
            float(os.getenv("OFFLINE_SAMPLE_INTERVAL", "8.0")),
        )
        ...
        self._block_rest_on_dns_domain = os.getenv("NETWORK_OUTAGE_BLOCK_REST_SAME_DOMAIN", "0").lower() in {
            "1",
            "true",
            "yes",
            "on",
        ...
        offline_flag = (
            os.getenv("MARKET_FORCE_OFFLINE")
            or os.getenv("MARKET_OFFLINE_ONLY")
            or os.getenv("NETWORK_FORCE_OFFLINE")
        )
        self._offline_only = str(offline_flag or "").strip().lower() in {"1", "true", "yes", "on"}
        self._last_offline_only_log = 0.0
        self._dns_failure_threshold = max(2, int(os.getenv("DNS_OUTAGE_THRESHOLD", "3")))
        self._dns_offline_base = max(15.0, float(os.getenv("DNS_OFFLINE_BACKOFF_SEC", "90")))
        self._dns_offline_growth = max(1.1, float(os.getenv("DNS_OFFLINE_GROWTH", "1.4")))
        self._dns_offline_max = max(self._dns_offline_base, float(os.getenv("DNS_OFFLINE_MAX_SEC", "360")))
        self._dns_log_interval = max(30.0, float(os.getenv("DNS_OFFLINE_LOG_INTERVAL", "60.0")))
        self._dns_failures = 0
        self._dns_offline_until = 0.0
        self._last_dns_offline_log = 0.0
        self._dns_log_symbol = self.symbol.upper()
        self._dns_probe_interval = max(0.0, float(os.getenv("DNS_PROBE_INTERVAL", "20.0")))
        self._dns_probe_timeout = max(0.05, float(os.getenv("DNS_PROBE_TIMEOUT", "1.0")))
        self._last_dns_probe = 0.0
        self._domain_outage_enabled = os.getenv("DOMAIN_OUTAGE_ENABLED", "1").lower() in {
            "1",
            "true",
            "yes",
            "on",
        }
        self._domain_outage_until: Dict[str, float] = {}
        self._domain_outage_failures: Dict[str, int] = {}
        self._domain_outage_reason: Dict[str, str] = {}
        ...
        self._endpoint_network_backoff_base = max(
            5.0,
            float(os.getenv("ENDPOINT_NETWORK_BACKOFF_SEC", "15")),
        )
        self._endpoint_network_backoff_growth = max(
            1.1,
            float(os.getenv("ENDPOINT_NETWORK_BACKOFF_GROWTH", "1.6")),
        )
        self._endpoint_network_backoff_max = max(
            self._endpoint_network_backoff_base,
            float(os.getenv("ENDPOINT_NETWORK_BACKOFF_MAX_SEC", "120")),
        )
        self._endpoint_network_failures: Dict[str, int] = {}
        self._endpoint_network_backoff_until: Dict[str, float] = {}
        ...
        if self._offline_only:
            ...
            if not self._ws_disabled_reason:
                self._ws_disabled_reason = "offline"
            self.url = None
        self._offline_enabled = self._offline_only or os.getenv("MARKET_OFFLINE_FALLBACK", "1").lower() in {"1", "true", "yes", "on"}
    ...
    def _should_log_dns_offline(self, now: float) -> bool:
        """Throttle DNS outage warnings across streams for the same symbol."""
        last_local = self._last_dns_offline_log
        last_symbol = self._dns_log_by_symbol.get(self._dns_log_symbol, 0.0)
        interval = self._dns_log_interval
        if now - last_local < interval or now - last_symbol < interval:
            return False
        self._last_dns_offline_log = now
            ...
            while not self._stop_event.is_set():
                ...
                if self._offline_only:
                    if now - self._last_offline_only_log >= 60.0:
                        log_message(
                            "market-stream",
                            "offline-only mode active; serving cached snapshots",
                            severity="warning",
                            details={"symbol": self.symbol},
                        )
                        self._last_offline_only_log = now
                    await self._dispatch_offline_snapshot()
                    self._seed_reference_from_cache(allow_network=False)
                    sleep_for = max(self._offline_sample_interval, self._cooldown_sleep_interval(now))
                    await asyncio.sleep(sleep_for)
                    backoff = min(backoff * 1.1, 300.0)
                    continue
                ws_failover_ready = self._ws_failover_ready(now)
                if self._network_outage_active(now) and self._network_outage_source == "websocket":
                    if ws_failover_ready:
    ...
    async def _consume_ws(self) -> None:  # pragma: no cover - requires network
        async with aiohttp.ClientSession() as session:
            ...
            async with session.ws_connect(self.url) as ws:
                ...
                self._clear_domain_outage(_domain_key(_extract_host(self.url)))
    ...
    async def _poll_rest_data(self, duration: float) -> None:
        ...
        if self._offline_only:
            await self._dispatch_offline_snapshot()
            sleep_for = max(self._offline_sample_interval, self._cooldown_sleep_interval(time.time()))
            await asyncio.sleep(sleep_for)
            return
        ...
        while not self._stop_event.is_set() and time.time() < end_time:
            ...
            if remaining <= 0:
                break
            dns_offline = self._dns_offline_active(now)
            if self._network_outage_reason == "dns" and not dns_offline:
                try:
                    await self._probe_dns_recovery(now=now)
                except Exception:
                    # Probing is best-effort; continue with existing backoff if it fails.
                    pass
            if dns_offline:
                if self._should_log_dns_offline(now):
                    log_message(
                        "market-stream",
                        "DNS outage window active; serving offline snapshots",
                        severity="warning",
                        details={
                            "symbol": self.symbol,
                            "resume_in": round(max(0.0, self._dns_offline_until - now), 2),
                        },
                    )
                await self._dispatch_offline_snapshot()
                ...
                if remaining <= 0:
                    break
                cooldown = self._cooldown_sleep_interval(time.time())
                sleep_for = min(max(self._offline_sample_interval, cooldown), remaining)
                await asyncio.sleep(sleep_for)
                continue
            ...
            if not endpoints:
                ...
                if remaining <= 0:
                    break
                await asyncio.sleep(min(self._cooldown_sleep_interval(time.time()), remaining))
                continue
            attempts = 0
            total_attempted = 0
            ...
            for idx in range(0, len(endpoints), self._rest_parallel):
                ...
                for endpoint, result, latency in results:
                    ...
                    if result.error in {"dns", "network", "timeout"}:
                        ...
                        self._record_endpoint_network_failure(endpoint.name, reason=result.error)
                    ...
                    if result.error == "rate_limited_local":
                        continue
                    self._clear_endpoint_network_backoff(endpoint.name)
            ...
            if remaining <= 0:
                break
            await asyncio.sleep(min(self.rest_poll_interval, remaining))
    ...
    def _eligible_rest_endpoints(self) -> List[Endpoint]:
        ...
        for endpoint in self._ranked_endpoints():
            ...
            if resume and resume > now:
                continue
            network_resume = self._endpoint_network_backoff_until.get(endpoint.name, 0.0)
            if network_resume and network_resume > now:
                continue
            if self._domain_outage_active(self._endpoint_rest_domain(endpoint), now):
                continue
            eligible.append(endpoint)
        return eligible

    def _eligible_ws_endpoints(self, *, exclude: Optional[str] = None) -> List[Endpoint]:
        now = time.time()
        eligible: List[Endpoint] = []
        base, quote = self._rest_base, self._rest_quote
        for endpoint in self._ranked_endpoints():
            if exclude and endpoint.name == exclude:
                continue
            ...
            if network_resume and network_resume > now:
                continue
            if self._domain_outage_active(self._endpoint_ws_domain(endpoint), now):
                continue
            if _render_ws(endpoint, base, quote):
                eligible.append(endpoint)
        return eligible

    def _ws_failover_ready(self, now: Optional[float] = None) -> bool:
        if self._network_outage_source != "websocket" or self._network_outage_reason != "dns":
            return False
        now = now or time.time()
        outage_endpoint = self._endpoint_by_name(self._network_outage_endpoint)
        outage_domain = self._endpoint_ws_domain(outage_endpoint)
        for endpoint in self._eligible_ws_endpoints(exclude=self._network_outage_endpoint):
            domain = self._endpoint_ws_domain(endpoint)
            if outage_domain and domain == outage_domain:
                continue
            return True
        return False
    ...
    def _candidate_dns_hosts(self) -> List[str]:
        hosts: List[str] = []
        endpoint = self._endpoint_by_name(self._network_outage_endpoint)
        if endpoint:
            ws_host = _extract_host(_render_ws(endpoint, self._rest_base, self._rest_quote))
            rest_host = _extract_host(_render_rest(endpoint, self._rest_base, self._rest_quote))
            for host in (ws_host, rest_host):
                if host:
                    hosts.append(host)
        if not hosts:
            for endpoint in self.endpoints:
                host = _extract_host(_render_rest(endpoint, self._rest_base, self._rest_quote))
                if host:
                    hosts.append(host)
        return hosts

    async def _probe_dns_recovery(self, *, now: Optional[float] = None) -> bool:
        if self._network_outage_reason != "dns":
            return False
        now = now or time.time()
        if now - self._last_dns_probe < self._dns_probe_interval:
            return False
        self._last_dns_probe = now
        hosts = self._candidate_dns_hosts()
        if not hosts:
            return False
        for host in hosts:
            try:
                await asyncio.wait_for(
                    ...
                    timeout=self._dns_probe_timeout,
                )
                self._clear_network_outage()
                self._clear_rest_outage()
                self._clear_domain_outage(_domain_key(host))
                self._dns_failures = 0
                self._dns_offline_until = 0.0
                log_message(
                    "market-stream",
                    "DNS recovered; resuming live market polls",
                    severity="info",
                    details={"host": host, "symbol": self.symbol},
                )
                return True
            except Exception:
                continue
    ...
    def _endpoint_rest_domain(self, endpoint: Optional[Endpoint]) -> Optional[str]:
        if not endpoint or not endpoint.rest_template:
            return None
        url = _render_rest(endpoint, self._rest_base, self._rest_quote)
        return _domain_key(_extract_host(url))

    def _endpoint_ws_domain(self, endpoint: Optional[Endpoint]) -> Optional[str]:
        if not endpoint:
            return None
        url = _render_ws(endpoint, self._rest_base, self._rest_quote)
        return _domain_key(_extract_host(url))

    def _domain_outage_active(self, domain: Optional[str], now: Optional[float] = None) -> bool:
        if not domain or not self._domain_outage_enabled:
            return False
        now = now or time.time()
        until = self._domain_outage_until.get(domain, 0.0)
        if until > now:
            return True
        if domain in self._domain_outage_until:
            self._domain_outage_until.pop(domain, None)
            self._domain_outage_reason.pop(domain, None)
            self._domain_outage_failures.pop(domain, None)
        return False

    def _register_domain_outage(self, *, domain: Optional[str], reason: str, backoff: float) -> None:
        if not domain or not self._domain_outage_enabled:
            return
        now = time.time()
        previous = self._domain_outage_until.get(domain, 0.0)
        self._domain_outage_until[domain] = max(previous, now + backoff)
        self._domain_outage_reason[domain] = reason
        self._domain_outage_failures[domain] = self._domain_outage_failures.get(domain, 0) + 1

    def _clear_domain_outage(self, domain: Optional[str]) -> None:
        if not domain or not self._domain_outage_enabled:
            return
        self._domain_outage_until.pop(domain, None)
        self._domain_outage_reason.pop(domain, None)
    ...
    def _dns_outage_blocks_rest(self, endpoint: Optional[str]) -> bool:
        ...
        if not rest_domains:
            return False
        if any(domain != ws_domain for domain in rest_domains):
            return False
        if not self._block_rest_on_dns_domain and any(host != ws_host for host in rest_hosts):
            return False
        return True
    ...
    def _register_network_outage(
        ...
        *,
        ...
        now = time.time()
        outage_active = self._network_outage_active(now)
        existing_source = self._network_outage_source
        if exc is not None:
            reason = _classify_network_error(exc)
        ...
        preserve_websocket = outage_active and existing_source == "websocket" and source != "websocket"
        ...
        if source == "websocket" and reason == "dns":
            domain = self._endpoint_ws_domain(self._endpoint_by_name(endpoint))
            self._register_domain_outage(domain=domain, reason=reason, backoff=backoff)
        self.metrics.feedback(
            "market_stream",
            severity=FeedbackSeverity.WARNING,
        ...
        if reason == "dns":
            ...
            exponent = min(6, max(0, self._dns_failures - 1))
            offline_backoff = min(
                self._dns_offline_max,
                self._dns_offline_base * (self._dns_offline_growth ** exponent),
            )
            if self._dns_failures >= self._dns_failure_threshold:
                # Enter offline window and defer DNS probes so we do not clear the
                # cooldown immediately after detecting an outage.
                self._dns_offline_until = max(self._dns_offline_until, now + offline_backoff)
                if self._last_dns_probe < now:
                    self._last_dns_probe = now
                if emit_log and self._should_log_dns_offline(now):
                    log_message(
                        "market-stream",
                        "DNS outage; serving cached snapshots until resolver recovers",
                        severity="warning",
                        details={
                            "symbol": self.symbol,
                            "endpoint": endpoint,
            ...
            self._dns_failures = 0
            self._dns_offline_until = 0.0
        if emit_log and now - self._last_network_outage_log >= 30.0:
            log_message(
                "market-stream",
    ...
    def _dns_offline_active(self, now: Optional[float] = None) -> bool:
        now = now or time.time()
        if self._dns_offline_until > now:
            return True
        if self._dns_offline_until:
            self._dns_offline_until = 0.0
            self._dns_failures = 0
        return False
    ...
    def _cooldown_sleep_interval(self, now: Optional[float] = None) -> float:
        ...
        if self._endpoint_network_backoff_until:
            next_ready_values.append(min(self._endpoint_network_backoff_until.values()))
    ...
    def rest_health(self) -> Dict[str, Any]:
        ...
        for endpoint in self.endpoints:
            ...
            scores = {
                ...
                "network_failures": self._endpoint_network_failures.get(name, 0),
                "latency_ms": None,
                "backoff_until": self._endpoint_backoff_until.get(name, 0.0),
                "network_backoff_until": self._endpoint_network_backoff_until.get(name, 0.0),
            }
            ...
            resume = scores["network_backoff_until"]
            if resume and resume <= now:
                scores["network_backoff_until"] = 0.0
    ...
    def _clear_endpoint_network_backoff(self, name: str) -> None:
        ...
        self._endpoint_network_backoff_until.pop(name, None)

    def _record_endpoint_network_failure(self, name: str, *, reason: str) -> None:
        failures = self._endpoint_network_failures.get(name, 0) + 1
        self._endpoint_network_failures[name] = failures
        exponent = min(6, max(0, failures - 1))
        backoff = min(
            self._endpoint_network_backoff_max,
            self._endpoint_network_backoff_base * (self._endpoint_network_backoff_growth ** exponent),
        )
        previous = self._endpoint_network_backoff_until.get(name, 0.0)
        self._endpoint_network_backoff_until[name] = max(previous, time.time() + backoff)
        if reason == "dns":
            domain = self._endpoint_rest_domain(self._endpoint_by_name(name))
            self._register_domain_outage(domain=domain, reason=reason, backoff=backoff)
        self.metrics.feedback(
            "market_stream",
            severity=FeedbackSeverity.WARNING,
            label="endpoint_network_failure",
            details={
                "endpoint": name,
                "reason": reason,
                "failures": failures,
        ...
        if failures in (3, 10):
            log_message(
                "market-stream",
                f"endpoint {name} network unstable; cooling down",
                severity="warning",
                details={"reason": reason, "failures": failures, "backoff_sec": round(backoff, 2)},
            )
    ...
    def _record_endpoint_success(self, name: str, *, price: Optional[float] = None, consensus: Optional[float] = None) -> None:
        ...
        if name in self._endpoint_network_backoff_until:
            self._clear_endpoint_network_backoff(name)
        self._clear_domain_outage(self._endpoint_rest_domain(self._endpoint_by_name(name)))
    ...
    def _seed_reference_from_cache(self, *, allow_network: bool) -> None:
        if self.reference_price:
            self.reference_price = max(self.reference_price, 1e-9)
            return
        snapshot_price = self._load_snapshot_reference(allow_network=allow_network)
        if snapshot_price:
    ...
    async def _refresh_reference_price(self) -> None:
        ...
        now = time.time()
        for endpoint in self._ranked_endpoints():
            if self._endpoint_backoff_until.get(endpoint.name, 0.0) > now:
                continue
            if self._endpoint_network_backoff_until.get(endpoint.name, 0.0) > now:
                continue
            result = await self._fetch_rest_price(endpoint, base, quote)
            if result.error == "unavailable":
                continue
        ...
        if outage_detected:
            ...
            self._clear_network_outage_if_rest()
        if self.reference_price is None:
            self._seed_reference_from_cache(allow_network=True)
        else:
            self.reference_price = max(self.reference_price, 1e-9)
    ...
    def _select_next_endpoint(self) -> None:
        ...
        for ep in candidates:
            if self._endpoint_backoff_until.get(ep.name, 0.0) > now:
                continue
            if self._domain_outage_active(self._endpoint_ws_domain(ep), now):
                continue
