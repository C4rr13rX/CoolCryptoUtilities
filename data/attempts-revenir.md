...
2026-09-11 Jet (pass 114, planner) -- Hypothesis: pass 111's buy_omens of 1, 9, 7 and 2 meant the brain was shy, and the fix was a topology or threshold change. FALSIFIED, node-free, over 150 eligible 3600s corpora. At the shipped threshold (OMEN_COST_MULTIPLE 1.5, 0.9750% on a 0.6500% round trip) the median corpus labels 14.46% of bars trough, so a 60-bar held-out window CONTAINS 8.7 troughs in total -- all four pass-111 counts sit inside that ceiling, so they were never evidence about the brain. RESULT: 30 trough calls needs ~208 bars at perfect recall, more at any real recall. Lowering the threshold is a measured trap: multiple 0.5 gives 18.57% (1.28x omens) for a label that no longer means the move paid its own round trip. SECOND RESULT, the ceiling with TRUE labels as the caller over 208 bars, both windows: UP 91 corpora buy n=2176 +3.0316%/trade, sell n=2358 +2.4000%, every-bar n=18928 +0.1062%; DOWN 59 corpora buy n=1506 +1.6396%/trade, sell n=1406 +1.8884%, every-bar n=12272 -0.9590%. The 100% precision there is BY CONSTRUCTION and is not a result; the content is that the headroom is large in both windows and that the SELL half is larger than the buy half in DOWN -- a half no experiment here has ever scored. Shipped trading/omen_scoreboard.py (renderer cannot emit a per-trade percentage without an n= on the same line; under 30 trades renders UNREADABLE), scripts/omen_trough_census.py, 7 tests. b1bbd12, gate 721/0. NEXT: score the sell half on a real held-out arm [3186a96e]; make the window a validated parameter [3366105d]; fix load_bars, which raises AttributeError on 62 of 629 corpus files and hides 714,673 bars in 35 of them [e7de7a9a].
2026-09-11 Cove pass114 -- HYPOTHESIS: the 'confusion refresh raised ... _train_lock' ERROR that judges live readiness on a cached report is a live-money risk. RESULT: it is NOT -- cause is candidate (2), tests/test_readiness_report.py::_pipeline_stub building TrainingPipeline.__new__ and reaching the refresh; bursts of exactly 3 in one second in agent-pass windows, fixture symbols interleaved, __new__ only under tests/, all 10 non-test sites call __init__. Intermittent because GuardianLease is acquired BEFORE the lock. BUT it masked a real production silence: live_readiness_report discarded ensure_confusion_fresh()'s return, which is False on 5 non-raising paths (asyncio offload = the normal case, single-flight, min-gap, lease/lock held, min_samples), so the gate judged on the cached report with NO log at all. Measured: confusion_matrices.json 5968s old vs the 900s max age it enforces, 6.6x, silent. FIX 61c0185: WARNING with the age, spaced 300s, counting suppressed verdicts; staleness fields on all 3 return paths; lock read via getattr and named at WARNING. NUMBERS: ERROR lines last 400KB 30 -> 0; gate 721/0; 6 tests, 4 assertions proven to fail against HEAD. NEXT: nothing in web/ reads confusion_age_sec -- surface it, so a live refusal shows the age of its evidence.

2026-09-11 Iris -- [45e24952] production's 25h checkpoint gap. HYPOTHESIS: the 179-byte brain.bin.tmp was a dead thread, a full disk, or the RAM floor. ALL THREE FALSIFIED: the tmp's mtime moved 01:45:16 -> 01:55:28 (exactly supervisor.toml's autocheckpoint_secs=600, so the thread is alive and on cadence), 37.7 GB free, and 7712 MB available against the 6144 MB floor. REAL CAUSE: the auto-checkpoint thread (crates/node/src/api.rs:906) calls brain.checkpoint() unconditionally while the manual /brain/checkpoint handler (brain_api.rs:5662) branches on uses_wbrain_storage(); production's neurons live in the wbrain store, so the thread serialises a borrowed in-RAM fabric view that cannot be written and dies 179 bytes in, every 600s, for 25 hours. SECOND, INDEPENDENT BUG: main.rs:281 built tracing with EnvFilter::from_default_env() and no fallback directive, and w1z4rd_supervisor.py never sets RUST_LOG -- so the failure was unobservable BY CONSTRUCTION. PID 16108 started 2026-09-09 18:03 and its stderr log had not grown since 09-09 08:48: 31.8 hours, zero lines. RESULT: fix committed (858ca7d + f924aab), test a_checkpoint_that_dies_mid_write_is_not_reported_as_success proven to fail against the old behaviour, cargo test -p w1z4rd-brain --lib persistence:: 3 passed 0 failed. NOT YET PROVEN: a successful checkpoint landing, because production runs a 31.8h-old binary and that needs a rebuild plus a live-node restart. TWO ITEM PREMISES WERE WRONG: /brain/stats answers (HTTP 200, 22.2s -- slow, not hung) and brain.wbrain is growing every few seconds, so learned state is not being lost. NEXT: get the operator's call on restarting production, then confirm the tmp is gone and an "auto-checkpoint saved" line appears.

2026-09-11 Jet (pass 115) -- Hypothesis: the self frames beat their null on trough in BOTH windows (pass 111) yet COST 0.81pp when queried, so the dilution law is a fact about the query mechanism and the frames should be used OUTSIDE it as a free abstention gate. Did: added --self-gate to scripts/omen_experiment.py -- fit_self_gate() buckets the TRAIN window's self_outcome/self_error_run frames by trough rate, freezes the zero-trough buckets at a support floor of 30 before the node is asked anything, prints the list, and scores gated and ungated books off the SAME predict() calls in one run with trough precision and n on both sides. Result: CODE ONLY, NO NUMBER. :8091 would not stay up (process gone within the minute against brain-data-p114-gale-2win), and production's :8090 is not scoreable against. 5 tests pass; the gate is opt-in and changes nothing sent to the node. Also established, and it is worth more than my own item this pass: the operator's ordered brain items 1 and 2 were ALREADY DONE before his note -- [10140855] by Cove pass 111 (f0d82e4) and [5ec44914] by Gale pass 113 (5c789ca/de68f7d). Next: bring :8091 up, re-run Gale's two pinned windows with --skip-train --self-gate, and report both or NO EDGE.

2026-09-11 Cove pass 115 -- [fa75fa1a] L2 over a sticky L1. HYPOTHESIS: the
pass-111 "no order-carrying L2 scheme can clear the 0.30 guard" block was a
true number measured on the wrong encoder, because the probe's table and exit
code both read plain relative banding while the live path ships hysteresis.
DID: promoted transition_motif into trading/omen_layers with L2_TRANSITION_STEPS=2
and L1_HYSTERESIS_MARGIN=0.5; made the probe's gate judge --gate-margin
(default = shipped margin) and print margin 0 as the control; added supported-group
counts (n>=20) beside distinctness. RESULT: L2_transitions 0.2633 DOWN / 0.2000
UP, clears in both; run-length REJECTED on its own number 0.7117/0.5983, failing
at every margin and step count; old sequence_motif 0.5317/0.4083. Supported
groups went 0 -> 3 (10.5%) DOWN and 0 -> 5 (21.9%) UP, so L2 is measurable for
the first time; UP's best is 4.21x the 14.6% base on n=26, above L1's own 2.46x,
and the same two symbols REVERSED read 28.6% on n=21 -- order carries outcome
...
2026-09-11 Cove -- [c6196eb2] Does the L1 motif rule pay anywhere in the 30-240 minute band, and do the 81% of windows that abstain share a pre-scoring property? Swept 30/60/120/240/720 minutes over 102 base corpora at a 3600s cadence and 30/60/120/240 over 27 at 300s, train 350 / purge = horizon / test 60, bands fitted on TRAIN and frozen, no node contacted. RESULT: negative at every horizon on both cadences -- no arm clears its own standard error, the 300s set called TWO trades in 108 window-horizons, and the abstention is a SUPPORT FAMINE not a market property: the median 300s corpus has 61 distinct L1 motifs on 350 train bars and ZERO of them reach min_support=20, the median 3600s corpus fits 2 motifs covering 18% of its train window. The selector question is CLOSED: five pre-scoring properties, cut fitted on a discovery half and read on a held-out half, and three of four horizons INVERTED (lift 0.75x, 0.77x, 0.93x). Also found and fixed a units bug that erased one of the two windows -- splitting UP/DOWN on the buy-every-bar net AFTER the 0.65% round-trip cost put 102 of 102 corpora in DOWN at 60m. NEXT: a coarser L1 encoder, or a train window long enough to support the vocabulary this one emits -- the horizon is not the lever. Report data/brain_experiments/L1-HORIZON-BAND.md

2026-09-11 Gale [3366105d] -- HYPOTHESIS: the omen experiment's held-out windows were too short to CONTAIN enough trough labels for a per-trade net to mean anything, so pass 111's buy_omens of 1/9/7/2 were never evidence about the brain. WHAT I DID: node-free census of every candidate held-out window on 0004_AERO-USDC (21,926 bars, 3600s, 720-min horizon, 0.9750% threshold) at --test 60 and --test 400, counting the trough labels each window contains; then made that ceiling a first-class part of omen_experiment's report and guarded the per-trade cells with it. RESULT: 656 of 656 sixty-bar windows are UNREADABLE, median 9 trough labels (predicted 8.7), worst window [21703,21763) holds ZERO trough labels while reading as a clean DOWN window at 11.7% up-rate; 97 of 97 four-hundred-bar windows are readable, median 60. buy_net_per_trade and sell_net_per_trade now read the string UNREADABLE when the window's label ceiling or the call count is under 30, with the two reasons recorded separately, and validate_report_readability refuses the old shape at write time beside validate_report_horizon. NEXT: the same floor on the other harnesses -- omen_scorable_windows defaults --test 60, query_path_probe and shape_mutations 120, layer_probe 180, agreement_census 200, which at this corpus's median is 9/17/26/29 trough labels, all four below the floor. Report: data/brain_experiments/HELDOUT-WINDOW-READABILITY-p118-gale.md

2026-09-11 Jet (senior) -- HYPOTHESIS: the price_mu head reads 489.8x its own training label because it is miscalibrated, so the fix is a retraining cycle ([2a503d95]). FALSIFIED, AND THE REAL CAUSE IS UPSTREAM OF TRAINING. WHAT I DID: instead of launching the training run the item asked for, I tested whether the served artifact had ever been trained at all, read-only and without loading TensorFlow -- unzip models/active_model.keras, open model.weights.h5 with h5py, and compare every kernel's observed standard deviation against the theoretical standard deviation of its own initialiser (glorot_uniform limit/sqrt(3); RandomUniform(-0.05,0.05) -> 0.0288675 for embeddings). RESULT: the artifact dated 2026-09-11 01:57 is UNTRAINED. 19 kernels, observed/theoretical median 0.9985, full range 0.95 to 1.01; 18 of 23 bias and 1-D vectors are EXACTLY all-zero; the only 5 exceptions are the 4 LayerNormalization gammas at exactly 1.000000 with std 0.000000 and the LSTM bias that Keras initialises non-zero by construction; the three embeddings read 0.028866, 0.028880, 0.028769. Every parameter sits on its initializer value -- not one gradient step has touched the file. model_versions holds 0 rows, consistent with the bootstrap writer ensure_active_model, which builds a fresh model and writes it straight to the served path with no training, no promotion decision and no registry call. So 489.8x is simply what a random output layer reads, and the 1800x regression across two identical probes with no code change between them was a rebuild in between. TWO CONSEQUENCES FOUND THE SAME WAY, both filed: (1) live readiness takes an absent-report fallback that grants ready=True because load_active_model() is non-None, i.e. because a FILE EXISTS, under LIVE_ALLOW_MINI_READY which defaults to 1, and then overwrites the false-positive rate and win rate it reports with threshold-satisfying values -- a fail-OPEN, currently held shut only by ghost_check and the sparse-wallet clause ([e40a45dc]); (2) the untrained placeholder SLOWS the loop that would train it -- trading/bot.py:11003 reads has_model from os.path.exists and :11004 then uses cadence 900.0s instead of fast_cadence 300.0s, a 3x slowdown entered precisely because a bootstrap file is on disk. Also corrected the operator's third open finding: the "confusion report 1789121569s old" is not a stale report, that number IS the unix epoch -- _last_confusion_refresh is initialised to 0.0 and was never stamped, which proves the in-memory report is EMPTY, while data/reports/confusion_matrices.json on disk is 150s old and is re-read only once, at __init__. NEXT: [821de434] before any retraining -- make every write of the artifact attributable, add a weights-only is-it-trained check, and repoint bot.py:11003 at it; only then spend a pass on the training cycle. Do not re-quote the direction-head AUC of 0.56-0.59 without re-measuring it: it was taken against this artifact, and a fixed random projection over autocorrelated features can read above chance.

2026-09-11 Cove (pass 119) -- [cf169af6] the live gate judged the POOLED book.
HYPOTHESIS: per-strategy judging was missing from the live gate, as it had been
from graduation. FALSIFIED IN THAT FORM: `_ghost_validation(strategy_id)` and
`_ghost_validation_for_live()` already judge per strategy -- but only over
`_live_gate_candidates()` = `StrategyLedger().approved_ids()`, which is EMPTY
(0 live-approved strategies), so it falls through to the pool. The gate is
pooled in exactly the state where pooling costs most.
WHAT I DID: added `_per_strategy_verdicts` to services/live_gate_map.py and a
ranked table to scripts/live_gate_map.py, over the population graduation uses
(services.tradeable_evidence.reconstruct -> the ledger's own _live_tradeable /
horizon / implausible-outcome predicates). Renamed the "net profit ex-BSTONK"
row to the guard it reports, `single_symbol_dependence`.
RESULT (numbers): 1 verdict -> 38. QUALIFIED ON ITS OWN BOOK: NONE -- no
verdict changed, as predicted. The refusal reason is the finding: 13 of 38 read
`stale_ghost_book`, NOT negative_margin. atf_static has 126 reconstructed
tradeable round trips against 11 inside the 48h gate window.
NEAR MISS WORTH RECORDING: I applied `ghost_reason_is_earned` to a
display-normalised reason. The strict path's reason is the EMPTY STRING, so
"ok" would have rejected every strategy that passed on its own merits -- the
same inversion that held live_trading_enabled=False on every bot for two days.
Caught by the test, not by reading.
NEXT: [dbbfea96] -- measure the age of the newest ghost exit per strategy and
establish whether `stale_ghost_book` is caused by the tick-driven exit schedule
...
2026-09-11 Gale (pass 120) -- HYPOTHESIS: the 120s dataset_warmup model task sharing the SequentialScheduler with data_ingest is what dropped the tick rate to 17/10min. DID: wrote scripts/seq_queue_budget.py, which reads the scheduler's own task_timeout events out of logs/system.log -- a timeout is an EXACT measurement of queue-held seconds because _execute joins the worker on the scheduler's own thread -- and cross-checked against 120,733 market-stream event-loop-block events. RESULT, NEGATIVE ON THE HYPOTHESIS: the model category does hold 93.36 s/10min (15.6% of the queue, all dataset_warmup, 28 timeouts in 6h), but loop blocks are NOT enriched inside its 120s join windows -- 17.9% inside against a 15.6% baseline coverage over 6h (1.15x), and 14.8% against 15.6% over four days (0.95x, BELOW background). So the model category explains 1-2 of the 39 missing ticks/10min, not 39. The event loop is blocked essentially all the time, mean 61.7s per block. THE REAL NUMBER: data_ingest, the FEED task, overran 346 times in 6h and its longest abandoned run is 6879s (114 min) during which the feed task is not running at all; the longest single loop block measured is 7393.1s and matches. SHIPPED ANYWAY, and not oversold: Task.detach, with dataset_warmup and candidate_training detached, because the join bought nothing -- the timeout never killed the worker, it only decided when to abandon it, and is_running() already blocks a second copy. Cycle wall at the real task shape 8.05s -> 0.19s (322s -> 7s at production scale) with the feed task still running. Also added live per-category held_sec and a stuck_for_sec timer to status(), because the log can only ever show a floor. NEXT: [3550bc30] -- name the call inside _download_supervisor.run_cycle() that wedges and give it a bounded deadline, and run the live 10-minute feed rate before/after, which pass 120 could not because production serves stale code until restarted and boots in 13 minutes.

2026-09-11 Iris (pass 120) -- HYPOTHESIS: L1 stickiness does not travel because L1_HYSTERESIS_MARGIN is a fraction of each band's own WIDTH and the widths are terciles fitted per stream per corpus, so what actually needs controlling -- the alphabet TURNOVER the layer above sees -- is left to whatever the terciles happen to give. DID: added solve_hysteresis_margin and l1_change_rate to trading/omen_layers.py (NOT to a probe), bisecting the real sticky_motifs encoder on the TRAIN frames until the change rate lands on a target, plus scripts/omen_margin_travel.py which measures fixed-0.50 against solved on 6 pairs over base and arbitrum at 3600s cadence, train 400 / purge 12 / test 300, every number read on a window the fit never saw. RESULT, POSITIVE ON THE ENCODER AND NEGATIVE ON THE EDGE: turnover spread across pairs 28.8pp (33.3-62.2%) -> 11.8pp (25.6-37.3%), and held-out L2 goes from 3 of 11 windows passing the 0.30 identifier guard to 8 of 11, supported groups at n>=20 from 9 to 13. Three of the five flips are DOWN windows and two are UP, so it is not an up-window flatter. BUT the guard is necessary, not sufficient: FIVE of eleven windows still have ZERO L2 groups at n=20 and best lift is 0.00x on six, so L2 still predicts nothing and I re-pointed no live path at the solver. Three windows still fail after solving, including ARB-WBTC UP which got WORSE (0.3667 -> 0.3833) because its fixed rate already sat below the target. EURC-USDC base has 4 candidate windows at a 0.0% trough base rate so its DOWN window is excluded and named -- Cove's pass-116 ARB-WETH finding reproducing on a second pair, so it is the selection and not one corpus. NEXT: the SUPPORT column, not the distinctness column -- [89693706], where an early 3-corpus smoke says median supported L1 groups goes 4.0 -> 25.5 and train-window coverage 39.1% -> 73.5% when train length alone goes 350 -> 2000 bars. Report: data/brain_experiments/L1-MARGIN-TRAVEL-pass120-iris.md, commit 14fe0d6.
2026-09-11 | Cove, pass 120 | part of [5c3b2189] | hypothesis: pass 117's mutation arm inverted because a mutation lands as another near-unique key, so the mutations only pay if something SHARED is emitted alongside them -- a canonical coarse descriptor of the z-normalised lookback path, which two frames of the same shape both carry. Measured NODE-FREE, because a descriptor->majority-label lookup fitted on the train half is the upper bound on what any substrate could learn from that stream alone.
RESULT: NEGATIVE, and consistently. AERO-USDC h12 on the pass-114 windows: exact 0.1800 vs a 0.3200 same-window baseline in UP (-14.00pp) and 0.2575 vs 0.2975 in DOWN (-4.00pp), 400 bars each, abstention 0.0000. Across the first 12 base corpora with per-corpus windows (most positive and most negative of the last ten 400-bar blocks, train chosen a horizon earlier): 0 of 8 scored corpora positive in both window classes, 0 positive in even one, mean lift -12.94pp UP and -10.09pp DOWN. Two sub-findings worth more than the headline: (a) the fine 8x5 descriptor reproduces the L1 SUPPORT FAMINE exactly -- 359 classes over 1200 anchors, median 3 each, ONE clears min_support=20, held-out abstention 1.0000 -- and coarsening to 3x2 fixes support completely (6 classes, whole window covered) without producing any edge, so support was never the binding constraint here; (b) mutation invariance at 3x2 is 0.9867 for deep_jitter but 0.5333 for deep_flatten and 0.3600 for deep_dilate at 0 label flips, so two of the three admitted mutations do NOT preserve the coarse shape and "same shape" was partly false for them. Also: PoolKind::Internal is still inert at head (0 matches across crates/; only behavioural PoolKind match is Action, brain.rs:7425) and the pool.rs:683 line the item cites is a comment inside InstructionIntentEncoding::atomize about the coding brain's @intent: encoding -- so the item cannot be built as written. Commits 15bc267, 6e36cde.
NEXT: do not spend a node arm on shape identity as a path descriptor -- the offline upper bound is negative on 8 of 8 corpora. If the shape family is pursued further, the question that is still open is whether shape predicts anything CONDITIONAL on a regime or a cadence, not unconditionally; and the cheap version of that is the same lookup keyed on (descriptor, regime) before any node time.
2026-09-11 | Cove, pass 120 addendum | part of [5c3b2189] | hypothesis: the unconditional shape negative leaves open whether shape predicts CONDITIONAL on regime, so key the same offline lookup on (shape, trailing regime) -- label_regime is strictly causal by its own contract.
RESULT: it does not rescue it. Still 0 of 8 corpora positive in both window classes; mean lift improves from -12.94/-10.09pp to -9.43/-8.55pp and stays negative, with 2 corpora positive in exactly ONE window, which the item's rule calls a FAIL. The finer key splits support as expected: chosen granularity drops to 2x2 on several tapes and abstention rises to 0.41/0.36 on VELVET-USDC. Commit 90f74e9.
NEXT: the shape family is answered offline in both its unconditional and its regime-conditional form -- do not spend a node arm on a shape-class collection without a class definition that differs from a coarse path descriptor. METHOD NOTE FOR THE NEXT AGENT: my first conditional run printed numbers IDENTICAL to the unconditional run because a python str.replace patch silently matched nothing -- this tree is CRLF and an 
 pattern never matches. Identical numbers across a changed knob means the knob is not wired.

2026-09-11 Jet (p120) -- Hypothesis: the shape-mutation arm inverted in p117 because eleven flat pools cannot represent "these two frames are the same shape", so carry the relation in its own pool. Did: verified PoolKind::Internal is INERT (0 matches across the W1z4rDV1510n crates; only behavioural pool-kind match is brain.rs:7425 PoolKind::Action), so sent a CLIENT-COMPUTED coarse shape key into pool 7 (news_entities -- declared and knobbed in the shipped v2 identity, never fed). Gated it node-free first over 400 anchors: fine key k8 76.8% distinct but only 39.2% of anchors in a shared key (the p117 failure repeating); coarse key k4 11.0% distinct with 99.5% of anchors in a supported key and 92.8% deep_jitter collision. Dropped deep_dilate (6.8% collision -- it clamps read positions at 0, so it is a CROP, not a tempo change). RESULT, NEGATIVE: the first arm was byte-identical to the no-pool arm on all four metrics over 400 samples because predict() streams a fixed PREDICT_COLLECTIONS -- the pool was trained and never queried. With it named in OMEN_PREDICT_COLLECTIONS, held-out exact 0.3550->0.2175 UP and 0.2625->0.2050 DOWN, every arm BELOW majority (0.4225 / 0.4300) in both windows. k4's 0.110 distinctness sits INSIDE the dilution law's empty band 0.103-0.260, so the drop is that law confirmed again. Cove reached the same NO independently across 8 corpora by a train-half frequency-table upper bound. Next: do not re-band this key -- support and distinctness are the same quantity traded off, so change the CONSTRUCTION ([4c3d75cd]), and first check whether earlier relation/meta arms were ever in their own query set ([d5776ccd]). Report: data/brain_experiments/p120-jet-shape-relation-pool.md. Commit 160da1d.

2026-09-15 Cove pass 123 -- HYPOTHESIS: the label-ceiling guard shipped in omen_experiment.py (pass 118) leaves four other harnesses free to publish a per-trade percentage their held-out window could not carry, and omen_layer_probe's own guard is dead code. DID: imported the shared heldout_readability/readability_cell into omen_layer_probe and omen_scorable_windows rather than reimplementing the floor; called guard_edge_for_report + validate_report_readability at write time in layer_probe (it was defined and never called, and referenced heldout_readability without importing it -- a NameError on first use); guarded each scorable_windows row against its OWN window and added a pooled block whose ceiling is additive across pooled windows; raised the --test defaults 200->208 (census), 180->208 (layer_probe), 60->208 (scorable_windows), all from MIN_READABLE_HELDOUT_BARS_AT_3600S rather than literals; split build_parser out of main in census and scorable_windows so the default is testable without a node. RESULT: 8 passed in tests/test_a_default_window_that_held_nine_troughs_cannot_quote_a_per_trade_net.py; two published layer-probe artifacts are refused on the CALL COUNT alone (layer-up-heldout +2.9030% on 9 calls, p119-gale-l2max-up -0.2235% on 7), and layer-down-heldout (-3.1416% on 109 calls) is UNDETERMINED because no published artifact recorded its trough census. NEXT: wire omen_query_path_probe (no per-trade cell at all -- only the label census and the --test default apply there) and omen_shape_mutations, then re-run one layer_probe arm to settle the UNDETERMINED ceiling with a measured number.

2026-09-15 Jet (auditor, pass 123) -- AUDIT of item 1fc31b2a, pool 18 temporal_scale. Hypothesis: the three criteria pass 114 reported MET actually establish that pool 18 reaches the fabric. RESULT: they do not. collection_distinctness (omen_brain.py:362) and discriminating_collections (:381) are pure offline string counts over the balanced train list and never open a socket, so criteria 1, 2 and 5 prove only that the ENCODING changed -- criterion 3 (QUERY PATH LIVE) is the only one that tests the seam and it is the one still unmet. Separate finding, and the more expensive one: omen_experiment.py:1201-1203 tells the operator to start the node on brains/market_predictor_v2.identity.toml, which declares 11 pools (ids 1-11) and NONE of 15-19; market_predictor_v4_meta.identity.toml declares 19 including id 18 at line 297. Started a node on v2 and /brain/stats returned pool_count=12, total_neurons=0. omen_brain.py:207-212 states the consequence: a v2 node answers 'unknown input pool id 15' and _consolidate marks the whole sample a MISS, so the run trains nothing and still prints a held-out number. Also: --corpus takes a PATH, not a symbol -- '--corpus AERO-USDC' dies FileNotFoundError at omen_experiment.py:192. NEXT: ship the identity guard in omen_experiment.py (hard-fail when OMEN_META_COLLECTIONS=1 meets a node that cannot hold pools 15-19) with tests/test_meta_collections_refuse_a_node_without_pool_18.py, THEN re-run criteria 3 and 4 on v4_meta. Report: data/brain_experiments/pool-18-identity-audit.md

2026-09-15 Gale (pass 123) -- Hypothesis: the model task dataset_warmup holds the sequential queue and starves the feed (item cee842c9). RESULT: REFUTED, and the real cause found. The feed is not slow, it is DARK and has been for 57.6 hours -- market_stream newest row 2026-09-13 10:45:47, news parquet stopped 11:00:19 the same morning. Production PID 588 is alive since 2026-09-09 18:34 (so it runs pre-fix code) and is NOT hung: 115 CPU-hours burned, RSS 143MB. The box is at 99% memory load, 0.24GB available of 31.78GB. ResourceGovernor reads SYSTEM-WIDE memory (resource_governor.py:203) and should_pause() returns True for HIGH and CRITICAL -- the feed priority -- whenever that is >= 0.90 (line 492-494), while _emergency_shed_load kills our own >150MB children every 20s (lines 304, 381). Shedding frees at most our 143MB against ~31.4GB held elsewhere, so the loop never converges and console.log shows hundreds of consecutive CRITICAL/EMERGENCY pairs. Line 254 already computes other_programs_mem_mb and the cascade never reads it. Report: data/feed-dark-governor-self-shed.md. NEXT: gate the HIGH/CRITICAL pause and the child-kill on our_rss vs _max_rss_mb so the guard keeps full force when our tree IS the hog, and separately find who holds the 31GB -- fixing the governor alone still leaves production on a box with 0.24GB free.

2026-09-15 Iris pass 423 -- active_model.keras had three writers and one registrar. Hypothesis: the served artifact is untrained because ensure_active_model's bootstrap writer drops a freshly BUILT model on the served path with no registry row, no calibration probe and no warning, and downstream reads its EXISTENCE as having a model. Did: registered the bootstrap and asset-vocab writes with provenance (db.register_model_version gained a provenance arg), warned at the bootstrap write, added probe_artifact_training/is_artifact_trained (weights-only, no TF), defined the missing refinement_cadence_for_artifact (bot.py called it while it existed nowhere -- a NameError in the refinement loop) and pinned all of it with tests/test_every_write_of_the_active_model_leaves_a_row.py. RESULT: models/active_model.keras probes 20 of 21 1-D parameter vectors bitwise on their initializer -> UNTRAINED; background refinement cadence on that artifact moves 900.0s -> 300.0s (3x faster), and model_versions goes from 0 rows to one row per write. 10 passed, 1 skipped. NEXT: importing keras under .venv kills the interpreter silently (exit 0, no traceback) -- that, not the cadence, is what stops a model from ever being trained on this box. Worth its own item.
