...
- **Adaptive ghost pair limit** – `GhostTradingSupervisor.build()` (`trading/selector.py:318-383`) hard-caps `pair_limit`. Make it dynamic: read `TrainingPipeline._horizon_bias`/`ghost_focus_assets()` and temporarily raise the limit when the guardian flags short-horizon deficits. That quickly widens ghost coverage, producing accurate signals (and checkpoints) for the under-served window without waiting for the next scheduler sweep.
